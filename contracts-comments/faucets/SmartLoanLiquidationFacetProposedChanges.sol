...
//                // TODO: Once finished, remove the ternary operator as it seems to be obsolete inside the else-if scope
//                needed = healingLoan ? config.assetsAmountsToRepay[i].amount : (config.assetsAmountsToRepay[i].amount - balance);
//            }
//
//            if (needed > 0) {
...
//            address(token).safeApprove(address(pool), config.assetsAmountsToRepay[i].amount);
//
//            repaidInUSD += config.assetsAmountsToRepay[i].amount * prices[i] * 10 ** 10 / 10 ** token.decimals();
//
//            pool.repay(config.assetsAmountsToRepay[i].amount);
//        }
//
...
//
//        SmartLoanLib.getNativeToken().deposit{value : amount}();
//        address(SmartLoanLib.getNativeToken()).safeTransfer(_to, amount);
//
