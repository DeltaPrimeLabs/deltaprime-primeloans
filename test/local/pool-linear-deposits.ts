...
import {LinearIndex, OpenBorrowersRegistry, Pool} from "../../typechain";

chai.use(solidity);

const {deployContract, provider} = waffle;

let depositIndex: LinearIndex;
...
describe('Pool with variable utilisation interest rates', () => {
    ...
    describe('should properly make multiple deposits', () => {
        beforeEach(async () => {
            await sut.deposit({value: toWei("1.0")});
            await time.increase(time.duration.years(1));
            expect(fromWei(await sut.balanceOf(owner.address))).to.be.closeTo(1.05, 0.000001);
        });

        it("should properly make another deposits", async () => {
            await sut.deposit({value: toWei("1.0")});
            expect(fromWei(await sut.balanceOf(owner.address))).to.be.closeTo(2.05, 0.000001);

            await sut.deposit({value: toWei("2.0")});
            expect(fromWei(await sut.balanceOf(owner.address))).to.be.closeTo(4.05, 0.000001);

            await sut.deposit({value: toWei("5.7")});
            expect(fromWei(await sut.balanceOf(owner.address))).to.be.closeTo(9.75, 0.000001);

            await sut.deposit({value: toWei("3.00083")});
            expect(fromWei(await sut.balanceOf(owner.address))).to.be.closeTo(12.75083, 0.000001);
        });

        it("should properly make another deposits with different time gaps", async () => {
            await sut.deposit({value: toWei("1.0")});
            await time.increase(time.duration.years(0.5));
            expect(fromWei(await sut.balanceOf(owner.address))).to.be.closeTo(2.10125, 0.000001);

            await sut.deposit({value: toWei("2.0")});
            await time.increase(time.duration.years(3));
            //2.10125 * 1.15(5% * 3 years)
            expect(fromWei(await sut.balanceOf(owner.address))).to.be.closeTo(4.7164375, 0.000001);

            await sut.deposit({value: toWei("5.7")});
            await time.increase(time.duration.months(3));
            //(4.7164375 + 5.7) * 1.25890410958 (90/365 * 5%)
            expect(fromWei(await sut.balanceOf(owner.address))).to.be.closeTo(10.5448593322, 0.000001);

            await sut.deposit({value: toWei("3.00083")});
            await time.increase(time.duration.years(1));
            //(10.5448593322 + 3.00083) * 1.05
            expect(fromWei(await sut.balanceOf(owner.address))).to.be.closeTo(14.2229737988, 0.000001);
        });
    ...
    describe("withdraw function", () => {
        ...
        it("should not allow to withdraw more than already on deposit after accumulating interest", async () => {
            ...
            expect(fromWei(await sut.connect(user).balanceOf(user.address))).to.be.closeTo(1.05, 0.00001);
        ...
        it("should allow to withdraw part of the deposit", async () => {
            ...
            expect(fromWei(await sut.balanceOf(owner.address))).to.be.closeTo(0.85, 0.00001);

            await sut.deposit({value: toWei("2.5")});
            await time.increase(time.duration.years(3));
            await sut.withdraw(toWei("1.3000"));
            expect(fromWei(await sut.balanceOf(owner.address))).to.be.closeTo(2.5525, 0.00001);

            await sut.deposit({value: toWei("3.7")});
            await time.increase(time.duration.years(3));
            await sut.withdraw(toWei("2.1400"));
            expect(fromWei(await sut.balanceOf(owner.address))).to.be.closeTo(5.050375, 0.000001);
        });
