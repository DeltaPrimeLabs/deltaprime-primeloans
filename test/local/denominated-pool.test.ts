...
describe("ERC20Pool ERC20 token functions", () => {
  ...
  describe("transfer", () => {
    ...
    it("should accumulate interest for one year", async () => {
      ...
      expect(fromWei(await sut.balanceOf(user1.address))).to.be.closeTo(0.5365999995, 0.000001);
      expect(fromWei(await sut.getBorrowed(user2.address))).to.be.closeTo(0.3366, 0.000001);

      expect(fromWei(await sut.getDepositRate())).to.be.closeTo(0.07858220, 0.000001);
      expect(fromWei(await sut.getBorrowingRate())).to.be.closeTo(0.12527394, 0.000001);
    });
  });
});
