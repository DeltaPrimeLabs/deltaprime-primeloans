...
import MockToken6DecimalsArtifact from "../../artifacts/contracts/mock/MockToken6Decimals.sol/MockToken6Decimals.json";
...
describe('Pool with variable utilisation interest rates and rewards', () => {
    describe('Depositing only (multiple depositors)', () => {
        ...
        before("Deploy Pool & DepositRewarderERC20 contracts", async () => {
            ...
            // Deploy tokens
            poolToken = (await deployContract(owner, MockToken6DecimalsArtifact, [[depositor1.address, depositor2.address, depositor3.address]])) as MockToken;
            console.log(`POOL TOKEN DECIMALS: ${await poolToken.decimals()}`)
        ...
        it("should deposit (depositor1, depositor2)", async () => {
            ...
            await poolToken.connect(depositor1).approve(pool.address, parseUnits("10.0", 6));
            await pool.connect(depositor1).deposit(parseUnits("10.0", 6));
            expect(await pool.balanceOf(depositor1.address)).to.equal(parseUnits("10.0", 6));
            console.log(`rewardPerToken 1: ${fromWei(await rewarder.rewardPerToken())}`)

            // Deposit to pool DEPOSITOR2 -> +10.0
            await poolToken.connect(depositor2).approve(pool.address, parseUnits("10.0", 6));
            await pool.connect(depositor2).deposit(parseUnits("10.0", 6));
            expect(await pool.balanceOf(depositor2.address)).to.equal(parseUnits("10.0", 6));
            console.log(`rewardPerToken 2: ${fromWei(await rewarder.rewardPerToken())}`)
        });

        it("should initialize rewarder", async () => {
            await rewarder.addDeposits([depositor1.address]);
            expect(await rewarder.balanceOf(depositor1.address)).to.equal(parseUnits("10.0", 6));
            expect(await rewarder.balanceOf(depositor2.address)).to.equal(parseUnits("10.0", 6));
            console.log(`rewardPerToken 3: ${fromWei(await rewarder.rewardPerToken())}`)
        });
        ...
        it("should set up rewards rewarder contract", async () => {
            ...
            console.log(`rewardPerToken 4: ${fromWei(await rewarder.rewardPerToken())}`)
            console.log(`reward rate: ${fromWei(await rewarder.rewardRate())}`)
        });
        ...
        it("[360 days] should deposit (depositor1) to pool + depositor2 via transfer and check rewards after some time", async () => {
            ...
            await poolToken.connect(depositor1).approve(pool.address, parseUnits("10.0", 6));
            await pool.connect(depositor1).deposit(parseUnits("10.0", 6));
            expect(await pool.balanceOf(depositor1.address)).to.equal(parseUnits("20.0", 6));
            expect(await pool.balanceOf(depositor2.address)).to.equal(parseUnits("10.0", 6));
            ...
            console.log(`rewardPerToken 5: ${fromWei(await rewarder.rewardPerToken())}`)
            // FastForward 180 days ahead (+180 days in total)
            await time.increase(time.duration.days(180));
            console.log(`rewardPerToken 6: ${fromWei(await rewarder.rewardPerToken())}`)
            // check rewards
            ...
            await pool.connect(depositor1).transfer(depositor2.address, parseUnits("5.0", 6));
            ...
            await pool.connect(depositor1).withdraw(parseUnits("15.0", 6));
            ...
            await pool.connect(depositor2).withdraw(parseUnits("15.0", 6));
        });
        ...
        it("[180 days] should deposit (depositor1, 2 & 3) to pool and check rewards after some time", async () => {
            ...
            await poolToken.connect(depositor2).approve(pool.address, parseUnits("5.0", 6));
            await pool.connect(depositor2).deposit(parseUnits("5.0", 6));

            // Deposit to pool DEPOSITOR3 -> +5.0
            await poolToken.connect(depositor3).approve(pool.address, parseUnits("5.0", 6));
            await pool.connect(depositor3).deposit(parseUnits("5.0", 6));
