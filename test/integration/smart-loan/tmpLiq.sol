...
contract SmartLoanLiquidationFacet is ReentrancyGuardKeccak, SolvencyMethodsLib {
    ...
    **/
    ...
    struct LiquidationConfig {
        ...
        IStakingPositions.UnstakePosition[] unstakePositions;
    ...
    * @param _assetsToRepay bytes32[] names of tokens provided by liquidator for repayment
    * @param _amountsToRepay utin256[] amounts of tokens provided by liquidator for repayment
    * @param _unstakePositions positions to be unstaked to repay liquidator
    * @param _liquidationBonus per mille bonus for liquidator. Must be lower than or equal to getMaxLiquidationBonus()
    **/
    function unsafeLiquidateLoan(bytes32[] memory _assetsToRepay, uint256[] memory _amountsToRepay,
        IStakingPositions.UnstakePosition[] memory _unstakePositions, uint256 _liquidationBonus) external payable nonReentrant {

        liquidate(
            LiquidationConfig({
        assetsToRepay : _assetsToRepay,
        amountsToRepay : _amountsToRepay,
        unstakePositions: _unstakePositions,
    ...
    * @dev This function uses the redstone-evm-connector
    * @param _assetsToRepay bytes32[] names of tokens provided by liquidator for repayment
    * @param _amountsToRepay uint256[] amounts of tokens provided by liquidator for repayment
    * @param _unstakePositions positions to be unstaked to repay liquidator
    * @param _liquidationBonus per mille bonus for liquidator. Must be lower than or equal to  getMaxLiquidationBonus()
    **/
    function liquidateLoan(bytes32[] memory _assetsToRepay, uint256[] memory _amountsToRepay,
        IStakingPositions.UnstakePosition[] memory _unstakePositions, uint256 _liquidationBonus) external payable nonReentrant {
    ...
    **/
    function liquidate(LiquidationConfig memory config) internal {
        DiamondStorageLib.setLiquidationInProgress(true);
        ...
        if (!healingLoan) {
            if (assetsValue > suppliedInUSD + bonus) {
        ...
        // done here so the staking rewards stay in the loan
        if (!healingLoan && assetsValue > suppliedInUSD + bonus) {
            //meaning any positions that cannot be transferred as ERC20 tokens
            repayFromPositions(config.unstakePositions, suppliedInUSD + bonus - assetsValue, msg.sender);
        }
        ...
        DiamondStorageLib.setLiquidationInProgress(false);
    }

    function repayFromPositions(IStakingPositions.UnstakePosition[] memory _unstakePositions, uint256 _maxUnstakedInUsd, address _to) private {
        uint256 repaidInUsd;

        for (uint256 i; i < _unstakePositions.length; i++) {
            address facet = DiamondHelper._getUnstakingFacetAddress(_unstakePositions[i].unstakeSelector, true);

            (bool successUnstaked, bytes memory result) = facet.delegatecall(abi.encodeWithSelector(_unstakePositions[i].unstakeSelector, _unstakePositions[i].amount, 0));

            if (successUnstaked) {
                IStakingPositions.UnstakedAmount memory unstakeResult = abi.decode(result, (IStakingPositions.UnstakedAmount));
                IERC20Metadata token = getERC20TokenInstance(unstakeResult.symbol, true);

                address(token).safeTransfer(_to, unstakeResult.amount);
                repaidInUsd += unstakeResult.amount * SolvencyMethodsLib.getPrice(unstakeResult.symbol)
                * 10 ** 10 / 10 ** token.decimals();
            }
        }

        require(repaidInUsd <= _maxUnstakedInUsd, "Cannot unstake more than needed for repayment");
    }
