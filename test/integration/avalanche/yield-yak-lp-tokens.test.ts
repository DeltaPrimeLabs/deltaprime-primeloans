...
describe('Smart loan', () => {
    ...
    describe('A loan with staking TJ LP tokens on YY', () => {
        ...
        before("deploy factory and pool", async () => {
            ...
            let assetsList = ['AVAX', 'USDC', 'TJ_WAVAX_USDC_LP', 'YY_TJ_USDC_WAVAX_LP'];
            ...
            tokensPrices = await getTokensPricesMap(assetsList.filter(el => !(['TJ_WAVAX_USDC_LP', 'YY_TJ_USDC_WAVAX_LP'].includes(el))), getRedstonePrices, []);

            // TODO: Add possibility of adding custom ABIs to addMissingTokenContracts()
            tokenContracts.set('TJ_WAVAX_USDC_LP', new ethers.Contract(TOKEN_ADDRESSES['TJ_WAVAX_USDC_LP'], lpABI, provider));
            tokenContracts.set('YY_TJ_USDC_WAVAX_LP', new ethers.Contract(TOKEN_ADDRESSES['YY_TJ_USDC_WAVAX_LP'], lpABI, provider));

            tokensPrices = await getTokensPricesMap(
                [],
                getRedstonePrices,
                [
                        {symbol: 'TJ_WAVAX_USDC_LP', value: tjLPTokenPrice},
                        {symbol: 'YY_TJ_USDC_WAVAX_LP', value: yyTJLPTokenPrice},
                    ],
                tokensPrices
            );
        ...
        it("should deploy a smart loan", async () => {
            ...
            tokensPrices = await getTokensPricesMap(['AVAX', 'USDC'], getRedstonePrices,
                [{
                symbol: 'TJ_WAVAX_USDC_LP',
                value: (await redstone.getPrice('TJ_WAVAX_USDC_LP', {provider: "redstone-avalanche-prod-1"})).value
            },
                {
                    symbol: 'YY_TJ_USDC_WAVAX_LP',
                    value: (await redstone.getPrice('YY_TJ_USDC_WAVAX_LP', {provider: "redstone-avalanche-prod-1"})).value
                }]);
        ...
        it("should fail to stake TJ LP tokens on YY", async () => {
            await tokenManager.deactivateToken(tokenContracts.get("TJ_WAVAX_USDC_LP")!.address);
            await tokenManager.deactivateToken(tokenContracts.get("YY_TJ_USDC_WAVAX_LP")!.address);
            await expect(wrappedLoan.stakeTJAVAXUSDCYak(1)).to.be.revertedWith("Token not supported");

            await tokenManager.activateToken(tokenContracts.get("TJ_WAVAX_USDC_LP")!.address);
            await expect(wrappedLoan.stakeTJAVAXUSDCYak(1)).to.be.revertedWith("Vault token not supported");

            await tokenManager.activateToken(tokenContracts.get("YY_TJ_USDC_WAVAX_LP")!.address);
        ...
        it("should stake TJ LP tokens on YY", async () => {
            ...
            let initialStakedBalance = await tokenContracts.get('YY_TJ_USDC_WAVAX_LP')!.balanceOf(wrappedLoan.address);
            ...
            let endStakedBalance = await tokenContracts.get('YY_TJ_USDC_WAVAX_LP')!.balanceOf(wrappedLoan.address);
    ...
    describe('A loan with staking TJ LP tokens on BeefyFinance', () => {
        ...
        before("deploy factory and pool", async () => {
            [owner, depositor] = await getFixedGasSigners(10000000);
            let assetsList = ['AVAX', 'USDC', 'TJ_WAVAX_USDC_LP', 'MOO_TJ_WAVAX_USDC_LP'];
            let poolNameAirdropList: Array<PoolInitializationObject> = [
                {name: 'AVAX', airdropList: [depositor]},
            ];
            ...
            await deployPools(smartLoansFactory, poolNameAirdropList, tokenContracts, poolContracts, lendingPools, owner, depositor)
            tokensPrices = await getTokensPricesMap(assetsList.filter(el => !(['TJ_WAVAX_USDC_LP', 'MOO_TJ_WAVAX_USDC_LP'].includes(el))), getRedstonePrices, []);

            // TODO: Add possibility of adding custom ABIs to addMissingTokenContracts()
            tokenContracts.set('TJ_WAVAX_USDC_LP', new ethers.Contract(TOKEN_ADDRESSES['TJ_WAVAX_USDC_LP'], lpABI, provider));
            tokenContracts.set('MOO_TJ_WAVAX_USDC_LP', new ethers.Contract(TOKEN_ADDRESSES['MOO_TJ_WAVAX_USDC_LP'], lpABI, provider));

            let lpTokenTotalSupply = await tokenContracts.get('TJ_WAVAX_USDC_LP')!.totalSupply();
            let [lpTokenToken0Reserve, lpTokenToken1Reserve] = await tokenContracts.get('TJ_WAVAX_USDC_LP')!.getReserves();
            ...
            let bfTotalSupply = await tokenContracts.get('MOO_TJ_WAVAX_USDC_LP')!.totalSupply();
            let bfTotalDeposits = await tokenContracts.get('MOO_TJ_WAVAX_USDC_LP')!.balance();
            ...
            tokensPrices = await getTokensPricesMap(
                ...
                [
                    {symbol: 'TJ_WAVAX_USDC_LP', value: tjLPTokenPrice},
                    {symbol: 'MOO_TJ_WAVAX_USDC_LP', value: yyTJLPTokenPrice},
                ],
                tokensPrices
            );
            addMissingTokenContracts(tokenContracts, assetsList);
        ...
        it("should fail to stake TJ LP tokens on YY", async () => {
            await tokenManager.deactivateToken(tokenContracts.get("TJ_WAVAX_USDC_LP")!.address);
            await tokenManager.deactivateToken(tokenContracts.get("MOO_TJ_WAVAX_USDC_LP")!.address);
            ...
            await tokenManager.activateToken(tokenContracts.get("MOO_TJ_WAVAX_USDC_LP")!.address);
        ...
        it("should stake TJ LP tokens on YY", async () => {
            let initialTJAVAXUSDCBalance = await lpToken.balanceOf(wrappedLoan.address);
            let initialStakedBalance = await tokenContracts.get('MOO_TJ_WAVAX_USDC_LP')!.balanceOf(wrappedLoan.address);
            const initialTotalValue = fromWei(await wrappedLoan.getTotalValue());
            totalValueBeforeStaking = initialTotalValue;
            ...
            let endTJAVAXUSDCBalance = await lpToken.balanceOf(wrappedLoan.address);
            let endStakedBalance = await tokenContracts.get('MOO_TJ_WAVAX_USDC_LP')!.balanceOf(wrappedLoan.address);
