...
const blockTimestampStart = 1693629600;
...
const sPrimeAirdropBorrowerArb = async (chain = 'arbitrum') => {
  ...
  while (timestampInSeconds <= blockTimestampEnd) {
    ...
    const factoryContract = new ethers.Contract(arbitrumFactoryAddress, FACTORY.abi, arbitrumHistoricalProvider);
    let loanAddresses = await factoryContract.getAllLoans({ blockTag: blockNumber });
    loanAddresses = loanAddresses.slice(0, 50)
    ...
    await Promise.all(
      Object.entries(loanDebts).map(async ([loanId, value]) => {
        ...
        const params = {
          TableName: 'sprime-airdrop-borrower-arb',
          Item: data
        };
...
sPrimeAirdropBorrowerArb('arbitrum');