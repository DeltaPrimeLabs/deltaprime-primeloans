...
contract YieldYakSwapFacet is ReentrancyGuardKeccak, SolvencyMethods {
    ...
    function yakSwap(uint256 _amountIn, uint256 _amountOut, address[] calldata _path, address[] calldata _adapters)
        ...
        recalculateAssetsExposure(_getAssets2(getInitialTokensDetails(_path[0], _path[_path.length - 1]).tokenBoughtSymbol, getInitialTokensDetails(_path[0], _path[_path.length - 1]).tokenSoldSymbol), _getPositions0())
        remainsSolvent
    {
