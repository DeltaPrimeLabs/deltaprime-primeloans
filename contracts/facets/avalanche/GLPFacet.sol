...
contract GLPFacet is ReentrancyGuardKeccak, OnlyOwnerOrInsolvent {
    ...
    function claimGLpFees() external nonReentrant onlyOwner noBorrowInTheSameBlock recalculateAssetsExposure(_getAssets1("AVAX"), _getPositions0()) remainsSolvent {
    ...
    function mintAndStakeGlp(address _token, uint256 _amount, uint256 _minUsdg, uint256 _minGlp)
        external
        nonReentrant onlyOwner noBorrowInTheSameBlock
        recalculateAssetsExposure(_getAssets2("GLP", DeploymentConstants.getTokenManager().tokenAddressToSymbol(_token)), _getPositions0())
        remainsSolvent
    {
    ...
    function unstakeAndRedeemGlp(address _tokenOut, uint256 _glpAmount, uint256 _minOut)
        ...
        recalculateAssetsExposure(_getAssets2("GLP", DeploymentConstants.getTokenManager().tokenAddressToSymbol(_tokenOut)), _getPositions0())
    {
        ITokenManager tokenManager = DeploymentConstants.getTokenManager();
