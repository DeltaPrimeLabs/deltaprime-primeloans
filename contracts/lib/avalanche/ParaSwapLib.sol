...
library ParaSwapLib {
    ...
    bytes4 private constant MEGASWAP_SELECTOR = 0x46c67b6d;
    ...
    function extractTokensAndAmount(bytes4 selector, bytes memory data) internal pure returns (address fromToken, address toToken, uint256 fromAmount) {
        ...
        } else if (selector == MEGASWAP_SELECTOR) {
            return getMegaSwapData(data);
    ...
    function getMegaSwapData(bytes memory data) internal pure returns (address fromToken, address toToken, uint256 fromAmount) {
        MegaSwapSellData memory swapData = abi.decode(data, (MegaSwapSellData));
        MegaSwapPath memory lastMegaSwapPath = swapData.path[swapData.path.length - 1];
        Path memory lastPath = lastMegaSwapPath.path[lastMegaSwapPath.path.length - 1];
        toToken = lastPath.to;
        (fromToken, fromAmount) = (swapData.fromToken, swapData.fromAmount);
    }
