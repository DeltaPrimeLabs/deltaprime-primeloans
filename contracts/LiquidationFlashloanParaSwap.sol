...
contract LiquidationFlashloanParaSwap is FlashLoanReceiverBase, Ownable {
    ...
    struct LiqEnrichedParams {
        ...
        uint256 bonus;
        IParaSwapRouter.SimpleData[] offers;
    }
   ...
   */
    function executeOperation(
        ...
        for(uint32 i = 0; i < assetSurplus.length; i++){
            if(assetSurplus[i].amount != 0){
                console.log('Asset: %s', assetSurplus[i].asset);
                console.log('Amount: %s', assetSurplus[i].amount);
            }
        }
        ...
        for(uint32 i = 0; i < assetDeficit.length; i++){
            if(assetDeficit[i].amount != 0){
                console.log('Asset: %s', assetDeficit[i].asset);
                console.log('Amount: %s', assetDeficit[i].amount);
            }
        }
                ...
                for (uint32 j = 0; j < assetSurplus.length; j++) {
                    if (assetSurplus[j].amount != 0) {
                        ...
                        for (uint32 k = 0; k < lep.offers.length; ++k) {
                            IParaSwapRouter.SimpleData memory offer = lep.offers[k];
                            if (
                                offer.fromToken == assetSurplus[j].asset &&
                                offer.toToken == assetDeficit[i].asset
                            ) {
        ...
        for (uint32 i = 0; i < assetSurplus.length; i++) {
            if (assetSurplus[i].amount != 0) {
                console.log('bonus transfer: %s', assetSurplus[i].asset);
                console.log('amount: %s', assetSurplus[i].amount);
        ...
        for (uint32 i = 0; i < assets.length; i++) {
            ...
            console.log('Approving: %s', assets[i]);
            console.log('Approve amount: %s', amounts[i] + premiums[i]);
            console.log('Balance amount: %s', IERC20Metadata(assets[i]).balanceOf(address(this)));
        }
    ...
    function getLiqEnrichedParams(
        ...
        uint256 _bonus;
        uint256 length;
        IParaSwapRouter.SimpleData[] memory _offers;
        assembly {
        ...
        _offers = abi.decode(encoded, (IParaSwapRouter.SimpleData[]));
        return
        LiqEnrichedParams({
            loan: _loan,
    ...
    function swapToNegateDeficits(
        ...
        IParaSwapRouter.SimpleData memory _offer
    ) private returns (bool shouldBreak, uint256 remainDeficitAmount) {
        console.log('WE ARE HERE 1');
        require(_offer.fromAmount > 0, "IParaSwapRouter offer fromAmount is 0");

        uint256 expectedBuyTokenReturned = (_offer.toAmount *
        _surplus.amount *
            98) / (_offer.fromAmount * 100);
...
//        console.log('_offer.fromAmount: %s', _offer.fromAmount);
        address(_surplus.asset).safeApprove(PARA_TRANSFER_PROXY(), 0);
        address(_surplus.asset).safeApprove(PARA_TRANSFER_PROXY(), _offer.fromAmount);

        uint256 beforeDeficitAmount = IERC20(_deficit.asset).balanceOf(
            address(this)
        );

//        _offer.fromAmount = amountIn;
...
//        });
        ...
        IParaSwapRouter router = IParaSwapRouter(PARA_SWAP_ROUTER());
        console.log('WE ARE HERE 3');
        router.simpleSwap(_offer);
...
//        router.swapNoSplit(trade, address(this), 0);
        ...
        _surplus.amount = _surplus.amount - _offer.fromAmount;
    ...
    function PARA_SWAP_ROUTER() internal virtual pure returns (address) {
        return 0xDEF171Fe48CF0115B1d80b88dc8eAB59176FEe57;
    }

    function PARA_TRANSFER_PROXY() internal virtual pure returns (address) {
