...
const uploadLiveLoansStatusAvalanche = async () => {
  ...
  console.log('totalLoans', loanAddresses.length);
  const batchSize = 100;

  const loanHistoryRef = db.collection('loansHistory');

  loanHistoryRef.listDocuments().then(async documentRefs => {
    const documentSnapshots = await db.getAll(...documentRefs);
    const totalLoans = documentSnapshots.length
    console.log('total loans saved', totalLoans);
    ...
    for (let i = 0; i < Math.ceil(totalLoans / batchSize); i++) {
      ...
      const batchDocumentSnapshots = documentSnapshots.slice(i * batchSize, (i + 1) * batchSize);

      await Promise.all(
        Array.from(batchDocumentSnapshots).map(async documentSnapshot => {
          // history[documentSnapshot.id] = {};

          const loanStatus = documentSnapshot.ref.collection('loanStatus');
          const statusRefs = await loanStatus.listDocuments();
          const statusSnapshots = await db.getAll(...statusRefs);

          await Promise.all(
            Array.from(statusSnapshots).map(async statusSnapshot => {
              if (statusSnapshot.exists) {
                ...
                // history[documentSnapshot.id][statusSnapshot.id] = statusSnapshot.data();
              }
            })
          )
        })
    ...
    const batch = 20;
    for (let i = 0; i < Math.ceil(history.length / batch); i++) {
      const batchHistory = history.slice(i * batch, (i + 1) * batch);

      let params = {
        RequestItems: {
          "loan-stats-ava-prod": batchHistory
        }
      };
...
// uploadLiveLoansStatusAvalanche()
...
const uploadLiveLoansStatusArbitrum = async () => {
  ...
  const loanHistoryRef = db.collection('loansHistoryArbitrum');
  ...
  loanHistoryRef.listDocuments().then(async documentRefs => {
    ...
    for (let i = 0; i < Math.ceil(history.length / batch); i++) {
      ...
      let params = {
        RequestItems: {
          "loan-stats-arb-prod": batchHistory
        }
      };
