  ...
  <div class="pools-table-row-component">
    <div class="table__row" v-if="pool" :class="{'unlocking': poolsUnlocking, 'disabled': pool.disabled}">
      <div class="table__cell asset">
        <img class="asset__icon" :style="{ borderRadius: '10px' }" :src="`src/assets/logo/${pool.symbol.toLowerCase()}.png`">
        <div class="asset__info">
          <div class="asset__name">{{ pool.symbol }}</div>
        </div>
      </div>
      ...
      <div class="table__cell table__cell--double-value tvl">
        ...
        <div class="double-value__usd">
          <span v-if="pool.tvl">{{ pool.tvlUsd | usd }}</span>
        </div>
      </div>
...
.pools-table-row-component {
  ...
  .table__row {
    display: grid;
    grid-template-columns: repeat(3, 1fr) 140px 140px 90px 90px 22px;
    height: 60px;
    border-style: solid;
